...
10. 简述一下Spring AOP
    ...
    主要分为静态代理和jdk动态代理，cglib代里 一个是编译期间进行增加（代理类和被代理类需要实现相同的接口或者父类），一个是调用期间（代理对象不需要实现接口,但是目标对象一定要实现接口,否则不能用动态代理）
    cglib（代理的类不能为final,否则报错，如果方法为static,private则无法进行代理，目标对象的方法如果为final/static,那么就不会被拦截,即不会执行目标对象额外的业务方法）
    主要是在方法前后，异常后进行增强操作
