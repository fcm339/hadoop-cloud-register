...
4. Mysql
       ...
       mysql如何实现和重复读：
           ...
           通过mvvc实现，通过在每行记录后面添加两个隐藏列，一个保存行的创建时间，可以称为事务版本好，一个保存行的过期时间，称为事务删除版本号。
           存储的不是实际的时间而是系统版本号。没次开启一个事务，系统版本号都会自动递增。事务开始时刻的系统版本
           号作为事务的版本号。
           
           更新操作：
                复制原来的行，并同时设置复制行的事务版本号为当前事务版本好，和原来行的事务删除版本号为当前事务版本号。
           删除操作：
           ...
           mvccc查询逻辑
                ...
                查询删除版本号为空，或者大于当前事务版本号的数据（保证的数据是事务开启前没有删除的数据）
